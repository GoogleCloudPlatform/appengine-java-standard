...
class RequestHandler extends Handler.Abstract {
  ...
  @Override
    ...
    Optional.ofNullable(request.Request(X_APPENGINE_USER_IP))
    ...
    Duration timeRemaining =
        Optional.ofNullable(request.Request(X_APPENGINE_TIMEOUT_MS))
  ...
  static boolean requestIsHttps(Request request) {
    if ("on".equals(request.getHeaders().get(X_APPENGINE_HTTPS))) {
      return true;
    }

    if ("https".equals(request.getHeaders().get(X_FORWARDED_PROTO))) {
      return true;
    }
