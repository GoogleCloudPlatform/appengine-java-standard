...
public class RequestManager implements RequestThreadManager {
  ...
  /**
   * {@code RequestToken} acts as a Memento object that passes state between a call to {@code
   * startRequest} and {@code finishRequest}. It should be treated as opaque by clients.
   */
  public static class RequestToken {
    /**
     * The thread of the request. This is used to verify that {@code finishRequest} was called from
     * the right thread.
     */
    ...
    /**
     * A collection of {@code Future} objects that have been scheduled on behalf of this request.
     * These futures will each be cancelled when the request completes.
     */
    private final List<Future<?>> scheduledFutures;
    ...
    /**
     * A {@code Timer} that runs during the course of the request and measures both wallclock and
     * CPU time.
     */
    ...
    public void addAppLogMessage(ApiProxy.LogRecord.Level level, String message) {
      upResponse.addAppLog(
          AppLogLine.newBuilder()
  ...
  /**
   * {@code DeadlineRunnable} causes the specified {@code Throwable} to be thrown within the
   * specified thread. The stack trace of the Throwable is ignored, and is replaced with the stack
   * trace of the thread at the time the exception is thrown.
   */
