...
public abstract class BaseCloudDatastoreV1ServiceImplTest {
  ...
  public final void setUpBase() throws Exception {
    ...
    when(cloudDatastoreV1Client.beginTransaction(any()))
        .thenReturn(Futures.immediateFuture(BeginTransactionResponse.getDefaultInstance()));
    when(cloudDatastoreV1Client.rawCommit(any()))
        .thenReturn(Futures.immediateFuture(CommitResponse.getDefaultInstance()));
    when(cloudDatastoreV1Client.rollback(any()))
                ...
                CompiledCursor
                    ...
                    .setAbsolutePosition(
                        IndexPosition.newBuilder()
                            .setKey(ByteString.copyFromUtf8("f" + cursorIndex.incrementAndGet()))
                            .build())
                    .build()
                    .toByteString())
