...
/** Tests for {@link LocalLoginServlet}. */
@RunWith(JUnit4.class)
public class LocalLoginServletTest {
  ...
  public void testContinueIsEscaped() throws Exception {
    ...
    when(req.getParameter("continue")).thenReturn(BAD_FORM_VALUE);
    when(resp.getWriter()).thenReturn(writer);

    new LocalLoginServlet().doGet(req, resp);
    writer.flush();
  ...
  public void testNoContinue() throws Exception {
    ...
    assertThat(buffer.toString()).contains("<input type='hidden' name='continue' value=''>");
  }
}
