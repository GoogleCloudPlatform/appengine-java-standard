...
public class LocalDatastoreCostAnalysis {
  ...
  private int changedIndexRows(EntityProto oldEntity, EntityProto newEntity) {
    ...
    if (oldEntity != null) {
      for (Property oldProp : oldEntity.getPropertyList()) {
        ...
        oldProp =
            oldProp.getMultiple()
                ? oldProp.toBuilder().clone().setMultiple(false).build()
                : oldProp;
    ...
    for (Property newProp : newEntity.getPropertyList()) {
      ...
      // we do this.
      newProp =
          newProp.getMultiple() ? newProp.toBuilder().clone().setMultiple(false).build() : newProp;
    ...
    for (Index index : allIndexes) {
      ...
      int ancestorMultiplier = index.getAncestor() && index.getPropertyCount() > 1 ? pathSize : 1;
      writes +=
  ...
  static List<Index> getEntityByPropertyIndexes(Set<String> propertyNames) {
    ...
    for (String propName : sortedPropertyNames) {
      ...
      Index.Builder index =
          Index.newBuilder()
              .setEntityType("") // Dummy value for mandatory field
              .setAncestor(false) // Dummy value for mandatory field
              .addProperty(
                  Index.Property.newBuilder().setName(propName).setDirection(Direction.ASCENDING));
      indexes.add(index.build());
      // EntitiesByPropertyDesc
      index =
          Index.newBuilder()
              .setEntityType("")
              .setAncestor(false); // Dummy values for mandatory field
      index.addProperty(
          Index.Property.newBuilder().setName(propName).setDirection(Direction.DESCENDING));
      indexes.add(index.build());
    }
    return indexes;
  }
